...
| Service  | Default | Returns               | Persistence       |
|----------|---------|-----------------------|-------------------|
...
- **qu.ax**: returns a permanent viewer page (HTML). Good for long-lived sharing; for direct embedding, prefer uguu.

## Quick Start

```powershell
# Upload a local image file (uses uguu first, then qu.ax on failure)
python scripts/upload.py "C:\path\to\image.png"

# Force a specific service (uguu | quax)
python scripts/upload.py "C:\path\to\image.jpg" --service quax
...
- Default order: **uguu** → **quax** (first success wins). Override with `--service`.
- Supports PNG, JPG, GIF, BMP, WEBP formats.
- uguu.se links are **temporary**; use `--service quax` for permanent links.
- Max file size: ~5 MB for qu.ax; ~128 MB for uguu.se.
